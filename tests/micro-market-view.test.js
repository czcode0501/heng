...
test("micro workspace renders unified ranges, candlesticks, order flow, profile and honest labels", () => {
  ...
  assert.match(html, /成交方向估算/);
  assert.match(html, /买方估算/);
  assert.match(html, /卖方估算/);
  assert.match(html, /历史成交密集区/);
  assert.match(html, /成交均价（估算）/);
  assert.match(html, /强弱指标（RSI 14）· 5分钟/);
  assert.match(html, /趋势动量（MACD 12,26,9）/);
  assert.match(html, /历史成交密集区（VRVP）/);
  assert.doesNotMatch(html, />PRICE × TECHNICAL EVIDENCE</);
