...
| 搜索、分析与添加持仓解耦 | 已完成 |
| 单股技术分析 | 已完成 |
| 创建多个投资组合 | 已完成，保存在当前浏览器本地存储；尚无云端同步 |
| 宏观信号中美分离 | 已完成 |
| 市场择时中美分离 | 已完成 |
| 板块轮动中美分离 | 已完成 |
| 投资者情绪中美分离 | 已完成 |
...
| 数据批量预载与共享缓存 | 已完成 |
| 最终五模型综合信号 | 尚未完成 |
| IBKR / QMT 券商连接 | 已完成只读快照；IBKR 总览可自动重连，尚无交易能力 |
| 用户登录与云端组合保存 | 尚未完成 |
| 自动交易与订单执行 | 不在当前版本范围 |

## 17. 测试与验证

截至 2026-08-23 的当前仓库自动验收：

- 109 个 Python 后端测试通过。
- 188 个 JavaScript 前端测试通过。
- Vite 生产构建通过。

最近一次记录在案的在线数据与浏览器人工验证日期仍为 2026-08-14：当时中美投资者情绪接口状态为 `live`，统一时间范围可以改变模型比较结果，桌面和窄屏布局无横向溢出，浏览器控制台未发现警告或错误。自动测试通过不能替代新一轮在线数据和浏览器人工验收。
